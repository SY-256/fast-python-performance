...
# 境界、None、ラップアラウンドのチェックを無効化する⇒indexのrange判定が不要
# 標準のPythonオブジェクトを渡さずに済むようにcdefを使う
# 戻り値の型としてCのvoidを宣言
@cython.boundscheck(False)
@cython.nonecheck(False)
@cython.wraparound(False)
cdef void live_core(
    ...
    for x in range(size_x):
        for y in range(size_x):
            ...
            if num_alive < 2 or num_alive > 3: # 近傍が少なすぎるまたは多すぎる場合
                new_world[y, x] = 0
            elif old_world[y, x] != 0: # 生き残る
                new_world[y, x] = old_world[y, x]
            elif num_alive == 3: # 生を受ける
                max_represented = max(states[1], max(states[2], max(states[3], states[4])))
                if max_represented > 1: # 色の多数決ルール
                    for i in range(1, 5):
