...
cdef void get_extended_world(
    ...
    extended_world[0, 1:x+1] = world[y-1, :]
    extended_world[y+1, 1:x+1] = world[0, :]
    extended_world[1:y+1, 0] = world[:, x-1]
    extended_world[1:y+1, x+1] = world[:, 0]
...
def live(cnp.ndarray[cnp.uint8_t, ndim=2] old_world):
    ...
    cdef cnp.ndarray[cnp.uint8_t, ndim=2] new_world = np.empty((size_y, size_x), dtype=np.uint8)
    cdef cnp.ndarray[cnp.uint8_t, ndim=1] states = np.empty((5,), dtype=np.uint8)
