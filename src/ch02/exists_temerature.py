...
def get_file_temeperature(file_name):
    with open(file_name, "rt") as f:
        ...
        for row in reader:
            ...
            if status != "1":
                continue
            temeperature = int(temperature) / 10
            yield temperature
...
def get_all_temperatures(stations, start_year, end_year):
    ...
    for station in stations:
        for year in range(start_year, end_year + 1):
            for temperature in get_file_temeperature(TEMPLATE_FILE.format(station=station, year=year)):
...
fisrt_all_temperatures = all_temperatures[stations[0]]
print(len(fisrt_all_temperatures), max(fisrt_all_temperatures), min(fisrt_all_temperatures))

# %timeit (-10.7 in fisrt_all_temperatures)
# %timeit (-100 in fisrt_all_temperatures)

set_first_all_temperatures = set(fisrt_all_temperatures)
