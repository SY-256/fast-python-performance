...
def map_less_native(executor, my_input, mapper):
    map_results = async_map(executor, mapper, my_input)
    return map_results

def map_reduce_less_native(my_input, mapper, reducer, callback=None):
    with Executor(max_workers=2) as executor:
        ...
        distributior = defaultdict(list)
        for key, value in map_results:
            distributior[key].append(value)
            
        futures = async_map(executor, reducer, distributior.items())
...
def reporter(tag, done, not_done):
    print(f"Operation {tag}: {done}/{not_done+done}")
    
words = "Python is great Python rocks".split(" ")
a = map_reduce_less_native(words, emitter, counter, reporter)
