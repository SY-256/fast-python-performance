...
vec_double_wo_overflow = np.vectorize(double_wo_overflow, otypes=[np.uint8])
...
def double_wo_overflow_1d(row):
    return np.minimum(np.multiply(2, row, dtype=np.uint16), 255)


vec_double_wo_overflow_1d = np.vectorize(
    double_wo_overflow_1d, otypes=[np.uint8], signature="(n)->(n)"
)
...
vec_double_wo_overflow_horrible = np.vectorize(
    double_wo_overflow_horrible, signature="(n)->(n)"
)

brighter_horrible_arr = vec_double_wo_overflow_horrible(image_arr)
print(
    brighter_horrible_arr.max(),
    brighter_horrible_arr.dtype,
    brighter_horrible_arr.shape,
)
